...
export const useStore = create(immer(
	...
	(set, get) => ({
		...
		remove(guitar: IGuitar): void {
			set(state => {
				state.guitars = state.guitars.filter(g => g._key !== guitar._key);
			});
		},
		changeScale(guitar: IGuitar, scale: IScale): void {
			set(state => {
				const ourGtr = state.guitars.find(g => g._key === guitar._key)!;
				ourGtr.scale = scale;
		...
		changeGauge(guitar: IGuitar, str: IString, gauge: IGauge): void {
			set(state => {
				const ourGtr = state.guitars.find(g => g._key === guitar._key)!;
				const strIdx = ourGtr.strings.findIndex(s => s._key === str._key);
