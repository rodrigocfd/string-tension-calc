...
const useStoreDef = defineStore('store', {
	state: () => ({
		unit: 'kg/cm' as IUnit,
		guitars: [] as IGuitar[],
	}),
	getters: {
		unit2: state => state.unit,
		guitars2: state => state.guitars,
	},
	actions: {
		...
		changePack(guitar: IGuitar, name: IPackName): void {
			...
			guitar.strings = genStrings(guitar.packName, guitar.tuningName, guitar.scale, this.unit);
		},
...
export type ReachableStore =
	StoreActions<ReturnType<typeof useStoreDef>> &
	StoreGetters<ReturnType<typeof useStoreDef>> &
	StoreState<ReturnType<typeof useStoreDef>>;
const useStore = (): ReachableStore => useStoreDef();
...
function genStrings(packName: IPackName, tuningName: ITuningName, scale: IScale, unit: IUnit): IString[] {
	...
	return pack.gauges.map((gauge, strIdx) => {
		...
		return {
			...
			tension: calcTension(strIdx, pack.gauges.length, gauge, tuning.notes[strIdx], scale, unit),
		};
	});
}
